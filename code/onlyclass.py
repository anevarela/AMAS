...
EXPECTED_SCHEMA = {
    # "ner_entities": ["list of strings matching entities"],
    "classification": {cat: "label" for cat in SDOH_CATEGORIES}
}
...
def build_system_prompt() -> str:
    ...
    return (
        ...
        - sdoh_economics / sdoh_environment: Clasificar según el texto (True/False/None).
        - behavior_alcohol / behavior_tobacco / behavior_drug: "Present" (activo actual, verbos en presente/adverbios), "Past" (histórico/cesación), o "None" (sin mención o negado). *Nota: drug es solo para drogas ilícitas o dependencia clínica, no medicamentos estándar.
        ...
        *Regla de fallecimiento: Si el paciente fallece al final de la nota, Job Status y Housing cambian a None; Alcohol/Tobacco/Drug cambian a "Past" si eran "Present".
        ...
        {
        "classification": {
            "sdoh_community_present": <categoría>,
            "sdoh_community_absent": <categoría>,
            "sdoh_education": <categoría>,
            "sdoh_economics": <categoría>,
            "sdoh_environment": <categoría>,
            "behavior_alcohol": <categoría>,
            "behavior_tobacco": <categoría>,
            "behavior_drug": <categoría>"
        }
        }
        """
    )
...
def validate_and_parse_json(raw_output: str) -> dict:
    ...
    if "classification" not in data:
        raise KeyError("Missing primary root keys ('classification').")
    for cat in SDOH_CATEGORIES:
        if cat not in data["classification"]:
            raise KeyError(f"Missing SDOH category classification key: {cat}")
    ...
    for model_name in MODEL_MAP.keys():
        ...
        for n_shot in [1]:
            ...
            try:
                ...
                test.to_csv(f"test_onlyclass_{n_shot}_{model_name}.csv", index=False)
